...
{
	gtime_t time = utc2gpst(timeget()); /* 获取当前时间，以gpst形式表示 */
	char path[1024];
...
static void udpos(rtk_t* rtk, double tt)
{
	...
	F = eye(nx);                            /* 状态转移矩阵 F */
	P = mat(nx, nx);                        /* 状态协方差矩阵 P */
	FP = mat(nx, nx);                       /* 中间过程矩阵：F*P */
	x = mat(nx, 1);                         /* 上一历元的最优估计状态向量：x */
	xp = mat(nx, 1);                        /* 当前历元的先验估计状态向量：xp */

	for (i = 0; i < 6; i++) F[i + (i + 3) * nx] = tt;                          /* ??状态矩阵中接收机速度V部分 */

	/* include accel terms if filter is converged */
	if (var < rtk->opt.thresar[1]) {
		for (i = 0; i < 6; i++) {
			F[i + (i + 6) * nx] = SQR(tt) / 2.0;                               /* ??状态矩阵中接收机加速度a部分 */
		}
	}
	...
	/* x=F*x, P=F*P*F+Q 卡尔曼滤波，计算出当前历元的先验状态xp */
	...
	/* process noise added to only acceleration 只对加速度添加过程噪声 */
	...
	int index, double* y, double* e, double* azel)
{
	...
	if (norm(rr, 3) <= 0.0) return 0;                                     /* 如果接收机几何距离 <0,返回0 */

	/* 将接收机坐标rr[i]（X, Y, Z）复制到局部变量rr_中 */
	for (i = 0; i < 3; i++) rr_[i] = rr[i];

	/* 如果opt中设置了tidecorr对接收机位置rr_进行地球潮校正 */
	if (opt->tidecorr) {
		tidedisp(gpst2utc(obs[0].time), rr_, opt->tidecorr, &nav->erp,
			opt->odisp[base], disp);
		for (i = 0; i < 3; i++) rr_[i] += disp[i];
	}
	/* 将接收机位置rr_(X, Y, Z)转成pos(L, L, H)中 */
	ecef2pos(rr_, pos);

	/* 根据当前历元的卫星观测数n进行遍历循环 */
	for (i = 0; i < n; i++) {
		/* 计算<卫星 -- 接收机>几何距离，存于r中；计算方位角/仰角，存于azel中 */
		if ((r = geodist(rs + i * 6, rr_, e + i * 3)) <= 0.0)continue;
		if (satazel(pos, e + i * 3, azel + i * 2) < opt->elmin)continue;

		/* 根据卫星的卫星号sat、方差var、卫星健康svh剔除卫星 */
...
static int ddres(rtk_t* rtk, const nav_t* nav, const obsd_t* obs, double dt, const double* x,
	...
	double* H, double* R, int* vflg, int* nf1)
{
	...
	bl = baseline(x, rtk->rb, dr);								/* 计算卫星—接收机的基线长度，输出长度bl，三维分量存于dr中 */
	ecef2pos(x, posu); ecef2pos(rtk->rb, posr);					/* 将基站、移动站的XYZ坐标转成LLH坐标，存于posr，posu当中 */

	/* 初始化单差协方差矩阵Ri，Rj、对流层校正向量tropu，tropr、速度校正向量dtdxu，dtdxr */
	Ri = mat(ns * nf * 2 + 2, 1); Rj = mat(ns * nf * 2 + 2, 1); im = mat(ns, 1);
	tropu = mat(ns, 1); tropr = mat(ns, 1);
	dtdxu = mat(ns, 3); dtdxr = mat(ns, 3);

	/* 将rtk中全部卫星,全部频段的伪距、相位残差置零 */
	for (i = 0; i < MAXSAT; i++) {
		for (j = 0; j < NFREQ; j++) {
	...
	for (m = 0; m < 5; m++) {
		...
		for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
			frq = f % nf;                                                 /* frq =  0(frq_1);  1(frq_2) */
			code = f < nf ? 0 : 1;                                        /* code = 0(phase);  1(pseudorange) */

			/* find reference satellite with highest elevation, set to i 选择仰角最高的卫星作为参考星 */
			for (i = -1, j = 0; j < ns; j++) {
				sysi = rtk->ssat[sat[j] - 1].sys;
				if (!test_sys(sysi, m) || sysi == SYS_SBS) continue;	          /* 验证卫星系统是否时m所指代的系统 */
				if (!validobs(iu[j], ir[j], f, nf, y)) continue;			      /* 验证y中双频伪距、相位均不为0 */
				if (i < 0 || azel[1 + iu[j] * 2] >= azel[1 + iu[i] * 2]) i = j;   /* 获取仰角el最高的移动站卫星的所引存于i中 */
			}
			if (i < 0) continue;

			/* calculate double differences of residuals (code/phase) for each sat */
			for (j = 0; j < ns; j++) {
				if (i == j) continue;											  /* 跳过参考星（仰角最高的星） */
				sysi = rtk->ssat[sat[i] - 1].sys;
				...
				if (!code) {
					...
					if (opt->ionoopt != IONOOPT_IFLC) {
						/* phase-bias states are single-differenced so need to difference them 相位还需要做模糊度双差 */
				...
				/* save residuals 保存*/
				if (code) rtk->ssat[sat[j] - 1].resp[frq] = v[nv];				/* 保存双差伪距残差 */
				else      rtk->ssat[sat[j] - 1].resc[frq] = v[nv];				/* 保存双差相位残差 */
	...
	/* double-differenced measurement error covariance 计算双差残差的协方差 */
...
static int ddmat(rtk_t* rtk, double* D, int gps, int glo, int sbs)
{
	...
	/* 1.遍历所有卫星，频率将fix标志置0 (1:fix,2:float,3:hold) */
	for (i = 0; i < MAXSAT; i++) {
		for (j = 0; j < NFREQ; j++) {
			rtk->ssat[i].fix[j] = 0;
		}
	}
	/* 2.对D中非模糊度元素所在的对角线元素置1 */
	for (i = 0; i < na; i++) D[i + i * nx] = 1.0;

	/* 3.根据卫星系统进行大循环 */
	for (m = 0; m < 5; m++) {		/* m=0:gps/sbs,1:glo,2:gal,3:bds,4:qzs */

		/* 3.1如果GPS，GLONASS，SBAS的标志符为0，则将nofix置1 */
		nofix = (m == 0 && gps == 0) || (m == 1 && glo == 0) || (m == 3 && rtk->opt.bdsmodear == 0);

		/* 3.2根据频率进行小循环 */
		for (f = 0, k = na; f < nf; f++, k += MAXSAT) {
			/* 3.2.1查找第一颗符合要求的卫星作为参考星 (i=state index, i-k=sat index) */
			...
			/* 3.2.2重新遍历所有卫星 (j=state index, j-k=sat index, i-k=first good sat) */
...
static int resamb_LAMBDA_chongzai(rtk_t* rtk, double* bias, double* xa, int gps, int glo, int sbs, FILE* fp) {
	prcopt_t* opt = &rtk->opt;
	...
	/* 1.根据定位模式、模糊度固定模式、验证阈值进行判定，若是则跳出模糊度固定 return 0 */
	if (rtk->opt.mode <= PMODE_DGPS || rtk->opt.modear == ARMODE_OFF || rtk->opt.thresar[0] < 1.0) {
		rtk->nb_ar = 0;
		return 0;
	}

	/* 2.如果定位方差过大，跳出模糊度固定以避免错误 return 0 */
	for (i = 0; i < 3; i++) {
	...
	/* 3.将单差状态转移矩阵D转成双差状态转移矩阵D'，获得双差模糊度 */
	D = zeros(nx, nx);
	if ((nb = ddmat(rtk, D, gps, glo, sbs)) < (rtk->opt.minfixsats - 1)) {
		errmsg(rtk, "not enough valid double-differences\n");
		free(D);
		return -1; /* flag abort */
	}
	rtk->nb_ar = nb;	/* nb：双差模糊度数量 */
	/* nx=# of float states, na=# of fixed states, nb=# of double-diff phase biases */
	ny = na + nb; y = mat(ny, 1); Qy = mat(ny, ny); DP = mat(ny, nx);
	b = mat(nb, 2); db = mat(nb, 1); Qb = mat(nb, nb); Qab = mat(na, nb); QQ = mat(na, nb), Qa = mat(na, na);
	...
	matmul("NN", ny, ny, nx, 1.0, DP, D, 0.0, Qy);			/* Qy = D'*P*D */
...
==================================================================================\n");
		fprintf(fp, "双差模糊度数量：\nnb=,%2d\n", nb);
		fprintf(fp, "双差模糊度：\nb=[,");						/* <1>输出双差模糊度b */
		for (int iy = na; iy < ny; iy++) {
			fprintf(fp, "%10.5f,", y[iy]);
			if (iy < ny - 1) fprintf(fp, "");
			else			fprintf(fp, "]\n");
		}

		fprintf(fp, "模糊度协方差矩阵：\nQb=[,");				/* <2>输出b的协方差矩阵Qb */
		for (int iQ = 0; iQ < nb; iQ++) {
			for (int jQ = 0; jQ < nb; jQ++) {
				if (iQ == 0 && jQ == 0)					fprintf(fp, "%10.5f,", Qb[iQ + jQ * nb]);
				else if (iQ == nb - 1 && jQ == nb - 1)  fprintf(fp, "%10.5f,]\n", Qb[iQ + jQ * nb]);
				else {
					if (jQ < nb - 1) {
						if (jQ == 0)	fprintf(fp, ",%10.5f,", Qb[iQ + jQ * nb]);
						else			fprintf(fp, "%10.5f,", Qb[iQ + jQ * nb]);
					}
					else				fprintf(fp, "%10.5f\n", Qb[iQ + jQ * nb]);
				}
			}
		}
#if 0
		...
		//	for (int jQ = 0; jQ < na; jQ++) {
		//		if (iQ == 0 && jQ == 0) {
		//			fprintf(fp, "%14.10f", Qa[jQ + iQ * na]);
		//		}
		//		else if (iQ == na - 1 && jQ == na - 1) {
		//			fprintf(fp, "\t%14.10f;]\n", Qa[jQ + iQ * na]);
		//		}
		//		else {
		//			if (jQ < na - 1) {
		...
		fprintf(fp, "==================================================================\
==================================================================================\n");
		fprintf(fp, "固定后的双差模糊度（最优解）：\nb1=[,");	/* <4>输出固定后的最优双差模糊度b1 */
		for (int iy = 0; iy < nb; iy++) {
			fprintf(fp, "%10.5f,", b[iy]);
			if (iy < nb - 1) fprintf(fp, "");
			else			fprintf(fp, "]\n");
		}
		fprintf(fp, "固定后的双差模糊度（次优解）：\nb2=[,");	/* <5>输出固定后的次优双差模糊度b2 */
		for (int iy = nb; iy < 2 * nb; iy++) {
			fprintf(fp, "%10.5f,", b[iy]);
			if (iy < 2 * nb - 1) fprintf(fp, "");
			else			fprintf(fp, "]\n");
		}
#endif
...
static int manage_amb_LAMBDA_chongzai(rtk_t* rtk, double* bias, double* xa, const int* sat, int nf, int ns, FILE* fp)
{
	int i, f, lockc[NFREQ], ar = 0, excflag = 0, arsats[MAXOBS] = { 0 };
	int gps1 = -1, glo1 = -1, gps2, glo2, nb, rerun, dly;
	float ratio1;
	...
	if (rtk->sol.prev_ratio2 < rtk->sol.thres && rtk->nb_ar >= rtk->opt.mindropsats) {
		...
		if (rtk->excsat < ar) {
			...
			for (f = 0; f < nf; f++) {
				lockc[f] = rtk->ssat[i - 1].lock[f];	/* save lock count */
				rtk->ssat[i - 1].lock[f] = -rtk->nb_ar; /* remove sat from AR long enough to enable hold if stays fixed */
			}
			trace(3, "AR: exclude sat %d\n", i);
			excflag = 1;
		}
	...
	if (rtk->opt.glomodear != GLO_ARMODE_FIXHOLD || rtk->holdamb) {
		...
		gps1 = 1;														/* GPS:		1.允许模糊度固定；0.不允许 (总是允许) */
		glo1 = rtk->opt.glomodear > GLO_ARMODE_OFF ? 1 : 0;				/* GLONASS: 1.允许模糊度固定；0.不允许 (根据设置) */
		/* 初次尝试固定模糊度 */
		nb = resamb_LAMBDA_chongzai(rtk, bias, xa, gps1, glo1, glo1, fp);
		ratio1 = rtk->sol.ratio;
		/* reject bad satellites if AR filtering enabled */
		if (rtk->opt.arfilter) {
			rerun = 0;
	...
	if (rtk->opt.glomodear == GLO_ARMODE_FIXHOLD) {
		...
		/* if modes changed since initial AR run or haven't run yet,re-run with new modes */
		if (glo1 != glo2 || gps1 != gps2)
			nb = resamb_LAMBDA_chongzai(rtk, bias, xa, gps2, glo2, glo2, fp);
	}
	/* restore excluded sat if still no fix or significant increase in ar ratio */
	if (excflag && (rtk->sol.ratio < rtk->sol.thres) && (rtk->sol.ratio < (1.5 * rtk->sol.prev_ratio2))) {
		i = sat[arsats[rtk->excsat++]];
...
static int relpos(rtk_t* rtk, const obsd_t* obs, int nu, int nr, const nav_t* nav)
{
	...
	rs = mat(6, n);            /* [X_s, Y_s, Z_s, Vx_s, Vy_s, Vz_s] */
	dts = mat(2, n);           /* [dts, ddts] */
	var = mat(1, n);           /* [var] */
	y = mat(nf * 2, n);        /* [res_L1, res_L2, res_P1, res_P2] */
	e = mat(3, n);             /* [Ix, Iy, Iz] */
	azel = zeros(2, n);        /* [az, el] */

	/* init satellite status arrays */
	for (i = 0; i < MAXSAT; i++) {
		rtk->ssat[i].sys = satsys(i + 1, NULL);                                           /* 给rtk中的卫星系统赋值 */
		for (j = 0; j < NFREQ; j++) {
	...
	satposs(time, obs, n, nav, opt->sateph, rs, dts, var, svh);                            /* <1>.求解卫星PVC */
	...
	if (!zdres(1, obs + nu, nr, rs + nu * 6, dts + nu * 2, var + nu, svh + nu, nav, rtk->rb, opt, 1,
		y + nu * nf * 2, e + nu * 3, azel + nu * 2)) {                                     /* <2>.对基站计算非差伪距、相位残差 */
	...
	if ((ns = selsat(obs, azel, nu, nr, opt, sat, iu, ir)) <= 0) {                         /* <3>.选择共视卫星，将共视卫星数量存于ns中 */
	...
	udstate(rtk, obs, sat, iu, ir, ns, nav);                                               /* <4>.更新状态 */
	...
	xp = mat(rtk->nx, 1);                          /* 当前历元的最优估计状态向量：xp(浮点解) */
	Pp = zeros(rtk->nx, rtk->nx);                  /* 当前历元的协方差矩阵：Pp */
	xa = mat(rtk->nx, 1);                          /* 模糊度固定后的最终状态向量：xa(整数解) */
	matcpy(xp, rtk->x, rtk->nx, 1);

	ny = ns * nf * 2 + 2;
	v = mat(ny, 1);                               /* 双差残差向量 */
	H = zeros(rtk->nx, ny);                       /* 观测矩阵 */
	R = mat(ny, ny);                              /* 观测协方差矩阵 */
	bias = mat(rtk->nx, 1);
	int nf1 = 0;
	...
	for (i = 0; i < niter; i++) {
		/* calculate zero diff residuals [range - measured pseudorange] for rover (phase and code)
			output is in y[0:nu-1], only shared input with base is nav
				...
				azel = [az, el] to sats                                   */
		trace(3, "rover:\n");
		if (!zdres(0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel)) {          /* <5>.对移动站计算非差伪距、相位残差 */
			errmsg(rtk, "rover initial position error\n");
			stat = SOLQ_NONE;
			break;
		}
		/* calculate double-differenced residuals and create state matrix from sat angles     <6>.双差观测及观测矩阵
				O rtk->ssat[i].resp[j] = residual pseudorange error
				O rtk->ssat[i].resc[j] = residual carrier phase error
				I dt = time diff between base and rover observations (usually 0)
		...
		if ((nv = ddres(rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, H, R, vflg, &nf1)) < 1) {
			errmsg(rtk, "no double-differenced residual\n");
			stat = SOLQ_NONE;
			break;
		}
		/* kalman filter measurement update, updates x,y,z,sat phase biases, etc              <7>.卡尔曼滤波校正，得到浮点解
				卡尔曼因子： K = P*H*(H'*P*H+R)^-1
				后验状态  ：xp = x+K*v
				后验协方差：Pp = (I-K*H')*P                                              */
		matcpy(Pp, rtk->P, rtk->nx, rtk->nx);
#if 1
		if (i == niter - 1) info = filter_chongzai(xp, Pp, H, v, R, rtk->nx, nv, &nf1, fp);  // YJR：为输出lambda算法参数修改
		else                info = filter(xp, Pp, H, v, R, rtk->nx, nv);
		if (info) {
			errmsg(rtk, "filter error (info=%d)\n", info);
			stat = SOLQ_NONE;
			break;
...
#endif
		/*else if ((info = filter(xp, Pp, H, v, R, rtk->nx, nv))) { 原代码
			errmsg(rtk, "filter error (info=%d)\n", info);
			stat = SOLQ_NONE;
			break;
		}*/
	...
	if (stat != SOLQ_NONE && zdres(0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel)) {
		/* 卡尔曼滤波后再次计算双差 */
		nv = ddres(rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, NULL, R, vflg, &nf1);
	...
	/* else resolve integer ambiguity by LAMBDA */
		...
		if (manage_amb_LAMBDA_chongzai(rtk, bias, xa, sat, nf, ns, fp) > 1) {
			...
			if (zdres(0, obs, nu, rs, dts, var, svh, nav, xa, opt, 0, y, e, azel)) {
				...
				nv = ddres(rtk, nav, obs, dt, xa, NULL, sat, y, e, azel, iu, ir, ns, v, NULL, R, vflg, &nf1);
...
extern int rtkpos(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav)
{
	...
	/* set base station position 确定基站坐标 */
	if (opt->refpos <= POSOPT_RINEX && opt->mode != PMODE_SINGLE && opt->mode != PMODE_MOVEB) {
		for (i = 0; i < 6; i++) rtk->rb[i] = i < 3 ? opt->rb[i] : 0.0;
	}
	/* count rover/base station observations */
	for (nu = 0; nu < n && obs[nu].rcv == 1; nu++);                              /* 计算移动站观测值个数 */
	for (nr = 0; nu + nr < n && obs[nu + nr].rcv == 2; nr++);                    /* 计算基准站观测值个数 */

	time = rtk->sol.time; /* previous epoch */

	/* rover position by single point positioning 通过单点定位计算出移动站坐标 */
