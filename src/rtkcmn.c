...
const prcopt_t prcopt_default = { /* defaults processing options */
	...
	0,1,1,1,0,0,                /* rcvstds,armaxiter,estion,esttrop,dynamics,tidecorr */
	...
	5E-12,                      /* sclkstab */
	{3.0,0.25,0.0,1E-9,1E-5,0.0,0.0,0.0}, /* thresar */ //{3.0,0.25,0.0,1E-9,1E-5,0.0,0.0,0.0}
...
extern double* mat(int n, int m)
{
	...
	if (!(p = (double*)malloc(sizeof(double) * n * m))) { /* 分配n*m个内存地址给p */
...
extern int filter(double* x, double* P, const double* H, const double* v, const double* R, int n, int m) {
	...
	/* 1. 状态向量x中非零元素的索引存于ix中，k存放非零元素计数 */
	ix = imat(n, 1);
	for (i = k = 0; i < n; i++) {
	...
	/* 3. 对原本的向量、协方差矩阵进行压缩，去除非零元素，存于新的矩阵中，节约运算时间 */
	for (i = 0; i < k; i++) {
		x_[i] = x[ix[i]];
	...
	/* 4. 在压缩后的矩阵中做卡尔曼滤波，得到后验状态xp_及协方差Pp_ */
	info = filter_(x_, P_, H_, v, R, k, m, xp_, Pp_);

	/* 5. 将更新后的xp_，Pp_中的元素重新返回x，P对应位置 */
	for (i = 0; i < k; i++) {
		x[ix[i]] = xp_[i];
...
extern int filter_chongzai(double* x, double* P, const double* H, const double* v,  /* YJR 重载卡尔曼滤波 */
	const double* R, int n, int m, int* nf1, FILE* fp)
{
	double* x_, * xp_, * P_, * Pp_, * H_;
	int i, j, k, info, * ix;
	...
	for (int iy = 0; iy < m; iy++) {
		fprintf(fp, "\t%15.10f", v[iy]);
		if (iy < m - 1) fprintf(fp, " ");
		else			fprintf(fp, "]\n");
	}

	fprintf(fp, "浮点位置坐标：\nb=[");					/* <3>输出浮点位置坐标[X Y Z] */
	for (int ix = 0; ix < 3; ix++) {
		fprintf(fp, "\t%20.10f", x_[ix]);
		if (ix < 2) fprintf(fp, " ");
		else		fprintf(fp, "]\n");
	}
...
#endif
	/*for (int iy = 0; iy < m; iy++) {
		for (int jy = 0; jy < 3; jy++) { fprintf(fp, "%10.10f ", *(H_ + jy + iy * k)); };
		fprintf(fp, ";\n");
	}*/
#if 0
	fprintf(fp, "模糊度系数矩阵：\nA=[");				/* <5模糊度系数矩阵A */
	for (int iy = 0; iy < m; iy++) {
		for (int jy = 3; jy < k; jy++) {
			fprintf(fp, "\t%14.10f", H_[jy + iy * k]);
			if (jy < k - 1)
				fprintf(fp, " ");
			else if (jy == k - 1 && iy < m - 1)
				fprintf(fp, ";\n");
			else
				fprintf(fp, ";]\n");
		};
	...
	//z0 = ((k - 3) / 2) - 1;
	//z1 = *nf1;
	//r2 = fabs(*(H_ + 3 + (*nf1) * k + z0 + (*nf1 - z0) + 1));
	//z2 = m / 2 - *nf1;
	//fprintf(fp, "%10.10feye(%d) zero(%d);", r1, z1, z1);
	//fprintf(fp, "zero(%d) %10.10feye(%d);", z2, r2, z2);
	//fprintf(fp, "zero(%d);", z1 + z2);
	//printf("%10.10feye(%d) 0;", r1, z1);
	//printf("0 %10.10feye(%d);", r2, z2);
	//fprintf(fp, "]\n");//输出 A  整周模糊度设计矩阵
	...
	fprintf(fp, "na=%2d\n", k - 3);
	fprintf(fp, "单差模糊度：\na=[");					/* <6>单差模糊度a */
	for (int ix = 3; ix < k; ix++) {
		fprintf(fp, "%10.5f", x_[ix]);
		if (ix < k - 1) fprintf(fp, " ");
		else		fprintf(fp, "]\n");
	}
#endif
	free(ix); free(x_); free(xp_); free(P_); free(Pp_); free(H_);
...
extern int readpcv(const char* file, pcvs_t* pcvs)
{
	...
	if (!(ext = strrchr(file, '.'))) ext = ""; /* 将file中“.atx”或“.ATX”赋给ext */
...
static int cmpobs(const void* p1, const void* p2)
{
	...
	if (fabs(tt) > DTTOL) return tt < 0 ? -1 : 1; /* 先判断两obs数据的时间差，如果超过DTTOL，则时间小的排前面 */
	if (q1->rcv != q2->rcv) return (int)q1->rcv - (int)q2->rcv;/* 再判断接收机号rcv，小的排前面 */
	return (int)q1->sat - (int)q2->sat; /* 最后判断卫星号sat，小的排前面 */
}
...
extern int sortobs(obs_t* obs)
{
	...
	qsort(obs->data, obs->n, sizeof(obsd_t), cmpobs); /* 给obs内的参数进行排序（时间差、rcv、sat） */

	/* delete duplicated data 删除重复数据 */
	for (i = j = 0; i < obs->n; i++) {
	...
	for (i = n = 0; i < obs->n; i = j, n++) { /* 计算obs中共有多少个历元 */
		for (j = i + 1; j < obs->n; j++) {
