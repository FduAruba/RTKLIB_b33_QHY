...
extern void eph2pos(gtime_t time, const eph_t* eph, double* rs, double* dts,
	double* var)
{
	double tk, M, E, Ek, sinE, cosE, u, r, i, O, sin2u, cos2u, x, y, sinO, cosO, cosi, mu, omge, n_, v;
	double xg, yg, zg, sino, coso;
	int n, sys, prn;

	trace(4, "eph2pos : time=%s sat=%2d\n", time_str(time, 3), eph->sat);

	/* 如果广播星历中的轨道长半轴 A <= 0.0，则对解算 rs 中的参数位置和钟差全部置0，退出运算 */
	if (eph->A <= 0.0) {
	...
	/* 根据卫星系统给mu（地球引力常数）和omge（地球自转角速度）赋值 */
	...
	/* <2>.计算卫星平均角速度 n_
	   <3>.计算卫星发射时间的平近点角 M */
	n_ = sqrt(mu / (eph->A * eph->A * eph->A)) + eph->deln;
	M = eph->M0 + n_ * tk;
	...
	/* <8>.计算校正后的升交点角距 u、卫星矢径长度 r、轨道倾角 i */
	...
	/* beidou geo satellite 北斗卫星校正及位置解算 */
	...
	*dts = eph->f0 + eph->f1 * tk + eph->f2 * tk * tk;
	...
	/* relativity correction  对卫星钟差dts进行相对论校正 */
	*dts -= 2.0 * sqrt(mu * eph->A) * eph->e * sinE / SQR(CLIGHT);

	/* position and clock error variance 构建位置和钟差的方差矩阵 */
...
static eph_t* seleph(gtime_t time, int sat, int iode, const nav_t* nav)
{
	...
	for (i = 0; i < nav->n; i++) {
		/*<1>.匹配卫星号 sat
		  <2>.匹配卫星数据期号 idoe（在ephpos中调用是为 -1）
		  <3>.判断卫星信号发射时间 time 和星历参考时间 toe 的间隔是否 > tmax ? 若间隔 < tmax 则执行下一步
		  <4>.如果 idoe >= 0 则返回当前星历
		  <5>.判断<3>中求得的间隔 t 是否 <= tmin ? 如果是则 tmin = t */
	...
	/* 返回在 idoe = -1 的情况下离信号发射时间 time 最近的星历 */
	return nav->eph + j;
}
...
static int ephclk(gtime_t time, gtime_t teph, int sat, const nav_t* nav, double* dts)
{
	...
	/* 通过卫星系统 sys 在导航数据nav中选择与星历选择时间标准teph最解近的toe给eph*/
	...
	int iode, double* rs, double* dts, double* var, int* svh)
{
	...
	*svh = -1;
	...
	if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS || sys == SYS_CMP) {
		...
		/* 通过确定顶的卫星星历来解算卫星的位置（x,y,z）和钟差dts */
		eph2pos(time, eph, rs, dts, var);
		/* 对卫星发射时间 + 微小时间间隔，再次解算位置和钟差，用来求卫星速度（vx,vy,vz）和频飘 */
	...
	/* satellite velocity and clock drift by differential approx 计算卫星速度和钟飘 */
	...
	int* svh)
{
	...
	*svh = -1; /* 若星历选择号ephopt不为以上，则卫星健康fla =-1，表示无法校正 */
	return 0;
}
	...
	int ephopt, double* rs, double* dts, double* var, int* svh)
{
	...
	for (i = 0; i < n && i < 2 * MAXOBS; i++) {
		/* 对解算 rs 、卫星钟差dts、方差var、卫星健康svh 初始化置0 */
		for (j = 0; j < 6; j++) rs[j + i * 6] = 0.0;
		for (j = 0; j < 2; j++) dts[j + i * 2] = 0.0;
		var[i] = 0.0; svh[i] = 0;

		/* search any psuedorange 遍历第i个卫星的频率个数，如果第j个频率的伪距为0，跳出循环*/
		...
		/* transmission time by satellite clock 用第i个卫星的接收时间 - 伪距信号传播时间 = 卫星信号发射时间 */
		time[i] = timeadd(obs[i].time, -pr / CLIGHT);

		/* satellite clock bias by broadcast ephemeris 计算第i个卫星的钟差 */
		if (!ephclk(time[i], teph, obs[i].sat, nav, &dt)) {
			trace(3, "no broadcast clock %s sat=%2d\n", time_str(time[i], 3), obs[i].sat);
			continue;
		}
		/* 用第i个卫星的发射时间 - 卫星钟差 = GPS时间下第i个卫星的信号发射时间 */
		time[i] = timeadd(time[i], -dt);

		/* satellite position and clock at transmission time 计算在GPS时间下卫星的PVC */
		if (!satpos(time[i], teph, obs[i].sat, ephopt, nav, rs + i * 6, dts + i * 2, var + i,
			svh + i)) {
			trace(3, "no ephemeris %s sat=%2d\n", time_str(time[i], 3), obs[i].sat);
			continue;
		}
		/* if no precise clock available, use broadcast clock instead 如果当前算出的卫星钟差为0，则再进行一次钟差计算 */
		if (dts[i * 2] == 0.0) {
