...
#define PATCH_LEVEL "b33f"               /* patch level */
...
/* type definitions ----------------------------------------------------------*/
	...
	/* QHY Defined Struct --------------------------------------------------------*/
	typedef struct { /* STEC value */
		gtime_t time;
		double stec_value[NSATCMP];
		double L1[NSATCMP];
		double L2[NSATCMP];
		double P1[NSATCMP];
		double P2[NSATCMP];
	...
	EXPORT int filter_chongzai(double* x, double* P, const double* H, const double* v,  //重载 为了输出lambda算法参数
