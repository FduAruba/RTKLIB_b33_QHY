...
static void decode_navh(char* buff, nav_t* nav)
{
	...
	trace(4, "decode_navh:\n");
	/* 根据label，结合不同的rnx版本，读取电离层校正参数a0,a1,a2,a3,b0,b1,b2,b3 */
	...
	char tobs[][MAXOBSTYPE][4], nav_t* nav, sta_t* sta)
{
	...
	*ver = 2.10; *type = ' '; *sys = SYS_GPS;
	...
	while (fgets(buff, MAXRNXLEN, fp)) { /* 每次读取一行 */
		if (strlen(buff) <= 60) continue;

		else if (strstr(label, "RINEX VERSION / TYPE")) {
			*ver = str2num(buff, 0, 9); /* 获取版本号 */
			*type = *(buff + 20); /* 获取rnx文件类型：N/O */

			/* satellite system */
			switch (*(buff + 40)) { /* 选择卫星、时间系统 */
			case ' ':
	...
	sta_t* sta)
{
	...
	/* set system mask */
	mask = set_sysmask(opt); /* 默认为选择全部卫星系统 */
...
/* return: 0:error; 1:succeed; -1:error */
...
/* return: 0:error; 1:succeed; -1:fail */
	...
	int* type, eph_t* eph, geph_t* geph, seph_t* seph)
{
	...
	/* set system mask 选择处理的卫星系统，若opt为空，则选择SYS_ALL */
	mask = set_sysmask(opt);

	while (fgets(buff, MAXRNXLEN, fp)) { /* 从fp指向的rnx文档中每次读取一行 */
		if (i == 0) {
			/* decode satellite field 解析卫星域，获取当前星历的卫星号sat及系统sys */
			...
			/* decode toc field */
			if (str2time(buff + sp, 0, 19, &toc)) { /* 将buff中的星历参考时间读出并存放到toc中 */
				trace(2, "rinex nav toc error: %23.23s\n", buff);
				return 0;
			}
			/* decode data fields */
			for (j = 0, p = buff + sp + 19; j < 3; j++, p += 19) { /* 将toc后的3个数据读入data */
				data[i++] = str2num(p, 0, 19);
			}
		}
		else {
			/* decode data fields 解析数据域，根据sat和sys获取具体星历数据，存于data中 */
			for (j = 0, p = buff + sp; j < 4; j++, p += 19) {
				data[i++] = str2num(p, 0, 19);
			}
			/* decode ephemeris */
			if (sys == SYS_GLO && i >= 15) {
				if (!(mask & sys)) return 0; /* 如果mask中的系统和当前卫星系统不一样，返回0 */
				*type = 1;
				return decode_geph(ver, sat, toc, data, geph); /* GLONASS解算星历入口 */
			}
			else if (sys == SYS_SBS && i >= 15) {
				if (!(mask & sys)) return 0;
				*type = 2;
				return decode_seph(ver, sat, toc, data, seph); /* SBAS解算星历入口 */
			}
			else if (i >= 31) { /* 读完一组32个星历数据 */
				if (!(mask & sys)) return 0;
				*type = 0;
				return decode_eph(ver, sat, toc, data, eph); /* GPS解算星历入口 */
			}
		}
	}
	return -1;
...
static int add_eph(nav_t* nav, const eph_t* eph)
{
	...
	if (nav->nmax <= nav->n) {
		...
		/* 给nav按照nmax来重新分配内存地址，并用nav_eph来获取地址 */
	...
	nav->eph[nav->n++] = *eph; /* 将eph中的数据传入nav */
	return 1;
}
...
{
	eph_t eph; /* GPS星历结构体 */
	geph_t geph; /* GLONASS星历结构体 */
	seph_t seph; /* SBAS星历结构体 */
	int stat, type;
	...
	/* read rinex navigation data body */
	/* 【！！】在此处通过while循环将导航文件中的星历全部输入至nav中 */
	while ((stat = readrnxnavb(fp, opt, ver, sys, &type, &eph, &geph, &seph)) >= 0) {
		/* add ephemeris to navigation data */
		if (stat) {
			switch (type) {
			case 1: stat = add_geph(nav, &geph); break; /* GLONASS */
			case 2: stat = add_seph(nav, &seph); break; /* SBAS */
			default: stat = add_eph(nav, &eph); break; /* GPS */
			}
			if (!stat) return 0;
		}
	}
	...
	obs_t* obs, nav_t* nav, sta_t* sta)
{
	...
	/* read rinex header 读 rnx 的头文件 */
	if (!readrnxh(fp, &ver, type, &sys, &tsys, tobs, nav, sta)) return 0;

	/* flag=0:except for clock,1:clock */
	if ((!flag && *type == 'C') || (flag && *type != 'C')) return 0;

	/* read rinex body 读 rnx 的数据 */
	switch (*type) {
	case 'O': return readrnxobs(fp, ts, te, tint, opt, index, ver, &tsys, tobs, obs, sta); /* 读.o文件入口 */
	case 'N': return readrnxnav(fp, opt, ver, sys, nav); /* 读.n文件入口 */
	...
	obs_t* obs, nav_t* nav, sta_t* sta)
{
	...
	/* uncompress file 解压缩文档 */
	if ((cstat = rtk_uncompress(file, tmpfile)) < 0) {
		trace(2, "rinex file uncompact error: %s\n", file);
		return 0;
	}
	if (!(fp = fopen(cstat ? tmpfile : file, "r"))) { /* 根据上一步解压的返回值cstat来打开文件 */
	...
	double tint, const char* opt, obs_t* obs, nav_t* nav, sta_t* sta)
{
	...
	for (i = 0; i < MAXEXFILE; i++) { /* 分配内存地址 */
		if (!(files[i] = (char*)malloc(1024))) {
			for (i--; i >= 0; i--) free(files[i]);
			return -1;
		}
	}
	/* expand wild-card 拓展文件地址通配符 */
	if ((n = expath(file, files, MAXEXFILE)) <= 0) {
		for (i = 0; i < MAXEXFILE; i++) free(files[i]);
		return 0;
	}
	/* read rinex files 读取rnx文档 */
